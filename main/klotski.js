if (typeof module === 'undefined') {
    var module = {}; // Impede erro no navegador
}
...
(function() {
  ...
  Klotski.prototype.exportLevelJSON = function(options, levelNumber = 1, mini = 81) {
  if (!options || !options.blocks) return null;

  const mapBlockType = (shape) => {
    const [h, w] = shape;
    if (h === 2 && w === 2) return "LARGE";
    if (h === 2 && w === 1) return "WIDE_VERTICAL";
    if (h === 1 && w === 2) return "WIDE_HORIZONTAL";
    if (h === 1 && w === 1) return "SMALL";
    return "UNKNOWN";
  };
  ...
  const blocks = options.blocks.map(block => {
    return {
      pos: {
        x: block.position[1], // coluna
        y: block.position[0], // linha
      },
      blockType: mapBlockType(block.shape)
    };
  });
